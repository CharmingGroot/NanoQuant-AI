"""
missed_profit.py - 모든 판단의 사후 추적 (옳고 그름 + LLM 학습 메모)

리플렉션 로직 통합: 가격 조회, pnl 계산, is_success 판정, LLM reflection_note 생성까지 한 번에 처리.
"""
...
def _get_is_success(action: str, pnl_pct: float) -> bool:
...
def run_decision_followup_cycle(
    ...
    for d in decisions:
        try:
            ...
            is_success = _get_is_success(action, pnl_pct)
